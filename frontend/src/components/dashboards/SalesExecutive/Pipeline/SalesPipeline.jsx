...
import Popup from "../../../commonpopup/popup"; // ✅ change path if needed

export default function SalesLeadPipeline() {
  ...
  const handleDrop = (targetColumnId) => {
    ...
    notify("success", `Lead moved to ${targetColumn.title}`);
    setDraggedLead(null);
  ...
  const handleAddDeal = (e) => {
    ...
    if (Object.keys(errors).length > 0) {
      notify("failure", "Please fix the form errors before submitting.");
      return;
    }
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 lg:px-15 max-w-full overflow-x-hidden">
      {/* ✅ Popup */}
      {popup.open && (
      ...
      {/* ✅ Pipeline Board */}
      {/* <div
       className="flex gap-3 sm:gap-4 overflow-x-auto pb-4 -mx-4 px-4"
      >
        {columns.map((column) => (
          <div key={column.id} className="min-w-0">
            <div className="rounded-xl border border-gray-200 shadow-sm bg-white h-full flex flex-col">
              <div className={`${column.color} rounded-t-xl p-4`}>
                <div className="flex items-center justify-between">
                  <h1 className="text-sm font-semibold">{column.title}</h1>
                  <div className="rounded-xl border-gray-400 border h-6 w-7 text-center bg-white">
              ...
              <div
                ...
                className="p-3 sm:p-4 space-y-3 min-h-[260px] sm:min-h-[360px] transition-colors flex-1"
              >
                {column.leads.map((lead) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="w-7 h-7 rounded-full bg-blue-100 sm:w-8 sm:h-8 flex-shrink-0 flex items-center justify-center">
