// SalesSidebar.jsx (FULL UPDATED CODE — heading moved downward + default active fixed)
...
const menuItems = [
  { name: "Dashboard", path: "/salesexecutive" },
  { name: "Leads", path: "/salesexecutive/lead-list" },
  { name: "Pipeline", path: "/salesexecutive/pipeline" },
  { name: "Calls", path: "/salesexecutive/call" },
  { name: "Payments", path: "/salesexecutive/payment" },
  { name: "Reports", path: "/salesexecutive/report" },
];

export default function SalesSidebar({ desktopVisible = true }) {
    ...
    <>
      ...
      {desktopVisible && (
        <aside
          className="
            ...
            md:px-4 md:pb-4 md:pt-8  /* ✅ heading pushed down */
            md:flex md:flex-col md:z-40
            md:h-[calc(100vh-4rem)]
          "
          aria-label="sales sidebar"
        >
              ...
              {menuItems.map((item) => (
                <li key={item.path}>
                  <NavLink
                    to={item.path}
                    className={navClass}
                    end={item.path === "/salesexecutive"} // ✅ prevents default active
                  >
                    {item.name}
                  </NavLink>
                </li>
      ...
      >
        ...
        <aside
          role="dialog"
          aria-modal="true"
          aria-label="Mobile sales sidebar"
          className="
            absolute left-0 top-0 bottom-0 bg-white shadow-lg
            transition-transform duration-300 ease-in-out flex flex-col
            px-4 pb-4 pt-16   /* ✅ heading pushed down below toggle */
          "
          style={{
            width: drawerWidth,
