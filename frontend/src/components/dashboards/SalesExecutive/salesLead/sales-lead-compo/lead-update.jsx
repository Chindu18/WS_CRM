...
// export default function UpdateLead({ isOpen, onClose, lead }) {
...
export default function SalesUpdateLead({ isOpen, onClose, lead, onUpdate }) {
  const [newStatus, setNewStatus] = useState("");
  const [priority, setPriority] = useState("");
  const [notes, setNotes] = useState("");
  const [errors, setErrors] = useState({});
  ...
  const handleSubmit = async () => {
    ...
    try {
      // ✅ call parent / API update if provided
      if (onUpdate) {
        await Promise.resolve(onUpdate(payload));
      } else {
        // fallback mock delay if onUpdate not passed
        await new Promise((resolve) => setTimeout(resolve, 1200));
      }

      setIsUpdating(false);

      // ✅ success popup (NO toast)
      openPopup(
        "success",
        "Lead Updated!",
                ...
                {expandedSections.status && (
                  <div className="p-4 space-y-3">
                    <select
                      ...
                      onChange={(e) => {
                        setNewStatus(e.target.value);
                        if (errors.newStatus) setErrors({ ...errors, newStatus: "" });
                      }}
                    >
                      <option value="">Select new status</option>
                      {statusOptions.map((o) => (
                    ...
                    {errors.newStatus && (
                      <p className="text-red-500 text-sm">⚠️ {errors.newStatus}</p>
                    )}
                  </div>
                )}
              </div>
