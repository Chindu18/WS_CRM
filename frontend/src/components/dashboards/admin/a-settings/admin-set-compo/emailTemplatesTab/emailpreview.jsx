...
export default function EmailTemplatePreview({
  ...
  onClose,
  template,
  openPopup, // ✅ optional global popup from parent
}) {
  const [isSending, setIsSending] = useState(false);

  // ✅ local popup fallback
  ...
  const safePopup = (type, title, message) => {
    if (openPopup) openPopup(type, title, message); // ✅ global popup
    else setLocalPopup({ open: true, type, title, message }); // ✅ local popup
  };
  ...
  const handleSendTest = async () => {
    ...
    // ✅ waiting popup
    safePopup("waiting", "Sending Test Email...", "Please wait a moment.");
    ...
    try {
      ...
      safePopup(
        "success",
        "Test Email Sent!",
        "A test email was sent successfully using this template."
      );
    } catch (err) {
      console.error(err);
      safePopup(
  ...
  return (
    <>
      {/* ✅ LOCAL POPUP (only if no global openPopup passed) */}
      {!openPopup && (
        <Popup
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="text-center">
                  <div className="mb-4">
                    ...
                    <p className="font-semibold text-gray-900">
                      The Team at Acme Inc
                    </p>
                  </div>
                  <div className="text-xs text-gray-500 space-y-1">
                    <p>
                      Acme Inc • 123 Business Street, Suite 100, San Francisco,
                      CA 94105
                    </p>
                    <p>
                      contact@acme-inc.com • www.acme-inc.com • +1 (555) 123-4567
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="flex items-center justify-between p-6 border-t border-gray-200">
            <div className="text-sm text-gray-500">
              This is a preview with sample data. Placeholders will be replaced
              with actual data when sent.
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={handleSendTest}
                disabled={isSending}
                className="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-xl font-medium transition-colors duration-200 disabled:opacity-60"
              >
                <Send className="w-4 h-4" />
