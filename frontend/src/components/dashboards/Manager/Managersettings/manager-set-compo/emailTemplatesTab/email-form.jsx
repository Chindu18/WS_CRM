...
const EmailTemplateModal = ({
  ...
  openPopup, // ✅ optional if parent wants to control popup globally
}) => {
  ...
  const safePopup = (type, title, message) => {
    if (openPopup) openPopup(type, title, message); // ✅ global popup if passed
    else setLocalPopup({ open: true, type, title, message }); // ✅ else local popup
  };

  const closeLocalPopup = () =>
    setLocalPopup((p) => ({ ...p, open: false }));
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ waiting popup
    safePopup("waiting", "Saving Template...", "Please wait a moment.");
    ...
    try {
      ...
      safePopup(
        "success",
        isCreateModalOpen ? "Template Created!" : "Template Updated!",
        isCreateModalOpen
          ? "Your new email template was saved successfully."
          : "Template changes were saved successfully."
      );
  ...
  return (
    <>
      {/* ✅ LOCAL POPUP (only used when openPopup not passed) */}
      {!openPopup && (
        <Popup
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex gap-2 flex-wrap">
                  {["[Name]", "[Company Name]", "[Email]", "[Phone]"].map(
                    (ph) => (
                      <button
                        key={ph}
                        type="button"
            ...
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-200">
              ...
              <button
                type="submit"
                disabled={isSubmitting}
                className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white px-6 py-3 rounded-xl font-semibold transition-all duration-200 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed"
              >
