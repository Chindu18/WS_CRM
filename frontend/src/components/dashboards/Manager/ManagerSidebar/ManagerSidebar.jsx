// ManagerSidebar.jsx (FULL UPDATED CODE — heading moved downward)
...
const menuItems = [
  ...
  { name: "Leads", path: "/manager/lead-list" },
  { name: "Pipeline", path: "/manager/pipeline" },
  { name: "Calls", path: "/manager/call" },
  { name: "Payments", path: "/manager/payment" },
  { name: "Reports", path: "/manager/report" },
  { name: "Automations", path: "/manager/automation" },
  { name: "Settings", path: "/manager/settings" },
];

export default function ManagerSidebar({ desktopVisible = true }) {
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bar top-4 left-4 z-[60] inline-flex items-center justify-center w-10 h-10 rounded-lg bg-white shadow
      ...
      {desktopVisible && (
        ...
        >
          ...
          <nav className="flex-1 bar-box overflow-y-auto pr-1 min-w-0">
              ...
              {menuItems.map((item) => (
                <li key={item.path} className="min-w-0">
                  <NavLink
                    to={item.path}
                    className={navClass}
                    end={item.path === "/manager"} // ✅ fixes default active
                  >
                    {item.name}
                  </NavLink>
                </li>
      ...
      >
        ...
        <aside
          role="dialog"
          aria-modal="true"
          aria-label="Mobile manager sidebar"
          className="
