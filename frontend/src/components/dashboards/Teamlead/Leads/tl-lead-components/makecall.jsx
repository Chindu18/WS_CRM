...
export default function MakeACall({ isOpen, onClose, lead = null, onSubmit }) {
  const SIZES = {
    modalMaxWidth: "max-w-md sm:max-w-lg lg:max-w-2xl",
    dialBtn: "w-16 h-16 sm:w-20 sm:h-20",
    iconDial: "w-7 h-7 sm:w-8 sm:h-8",
    actionBtn: "w-14 h-14 sm:w-16 sm:h-16",
  ...
  const handleStartCall = () => {
    if (!displayPhone.trim()) {
      openPopup(
        "failure",
        "Phone Required",
        "Please enter a valid phone number before starting the call."
      );
      return;
    }
    setStep("calling");
  ...
  // ✅ validate ended-form
  const validateEndedForm = () => {
    if (!result) {
      openPopup(
        "failure",
        "Call Result Missing",
        "Please select a call result before saving."
      );
      return false;
    }
    return true;
  ...
  const handleSubmitForm = async (e) => {
    e.preventDefault();
    if (!validateEndedForm()) return;

    // ✅ waiting popup
    openPopup("waiting", "Please wait...", "Saving call details...");
    ...
    openPopup(
      "success",
      "Call Saved!",
      `Call with ${newCall.leadName} saved successfully.`
    );
