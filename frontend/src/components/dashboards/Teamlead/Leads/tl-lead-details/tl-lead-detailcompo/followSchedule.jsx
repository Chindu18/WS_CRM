...
import Popup from "../../../../../commonpopup/popup"; // ✅ change path as needed

export default function TlFollowUpScheduler() {
  ...
  const handleFollowUpSubmit = (e) => {
    ...
    if (validateFollowUpForm()) {
      ...
      setUpcomingFollowUps((prev) =>
        [...prev, newFollowUp].sort((a, b) => a.datetime - b.datetime)
      );
      ...
      openPopup(
        "success",
        "Follow-up Scheduled!",
        "Your follow-up has been added successfully. 🗓️"
      );
    } else {
      openPopup(
        "failure",
  ...
  const handleDeleteFollowUp = (id) => {
    setUpcomingFollowUps((prev) =>
      prev.filter((followUp) => followUp.id !== id)
    );

    openPopup(
      "success",
      "Follow-up Deleted",
      "The follow-up was removed successfully."
    );
  };
          ...
          <form className="space-y-4" onSubmit={handleFollowUpSubmit}>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleFollowUpChange("time", e.target.value)
                }
                className={`w-full p-2 border rounded-xl ${
                  followUpErrors.time
                    ? "border-red-500 bg-red-50"
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 md:p-6">
          ...
          <div className="space-y-3">
            {upcomingFollowUps.map((followUp) => {
              const isOverdueFollowUp = isOverdue(
                followUp.date,
                followUp.time
              );
            ...
            {upcomingFollowUps.length === 0 && (
              <div className="text-center py-8">
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Schedule your first follow-up above
                </p>
              </div>
            )}
          </div>
